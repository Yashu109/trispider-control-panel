...
const ProtectedPayments = ({ projects, formatCurrency }) => {
  ...
  const [newSpend, setNewSpend] = useState({ amount: '', purpose: '' });
  ...
  const generateSpendSummary = () => {
    // Get the latest 3 spends for the summary
    const latestSpends = [...spends].sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 3);
    ...
    // const summary = latestSpends.map(spend => 
    //   `${formatCurrency(spend.amount)} for ${spend.purpose} (${new Date(spend.date).toLocaleDateString()})`
    // ).join(', ');
    
    // setSpendSummary(`Recent spends: ${summary}`);
  };

  const handleAddSpend = async () => {
    if (!newSpend.amount || !newSpend.purpose) {
      setError('Please enter both amount and purpose');
      return;
    }
          ...
          <div className="add-spend-form">
            ...
            <div className="spend-input-container">
              ...
              <button 
                ...
                className="Protected-cancel-button"
              >
                Cancel
              </button>
            </div>
        ...
        <div className="spends-table-container">
          ...
          <table className="spends-table">
            ...
            <tbody>
              ...
              {spends.length === 0 && (
                <tr>
                  <td colSpan="3" className="no-spends">No spends recorded yet</td>
                </tr>
              )}
            </tbody>
          </table>
    ...
    <div className="payments-dashboard">
      ...
      <div className="summary-cards">
        ...
        >
          ...
          {/* <DollarSign className="spend-icon" /> */}
